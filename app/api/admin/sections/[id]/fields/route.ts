...
const fieldSchema = z.object({
  key: z
    ...
    .regex(/^[a-z][a-z0-9_]*$/, "Key deve ser snake_case (ex: title, cover_image_url)"),
...
export async function POST(
  ...
  const existing = await prisma.sectionField.findUnique({
    where: { sectionId_key: { sectionId: params.id, key: parsed.data.key } },
  });
  if (existing) {
    return NextResponse.json({ error: "Já existe um campo com essa key nesta seção" }, { status: 422 });
  }
